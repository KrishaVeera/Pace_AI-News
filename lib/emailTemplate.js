...
function dropCell(drop, categoryLabel, padding, ACCENT, TEXT, MUTED, BG) {
  ...
  return `<td class="drop-col" style="vertical-align:top;padding:${padding};">
      <p style="margin:0 0 4px 0;font-family:'Libre Baskerville',Georgia,serif;font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:${MUTED};">${escapeHtml(categoryLabel)}</p>
...
export function buildDailyEmail(category, content) {
  ...
  const DDIV = `<td class="drop-divider"  style="width:1px;padding:0;background-color:${BORDER};"></td>`;
  ...
  const d1 = dropCell(content.drops?.coder,      'For Coders',          '0 20px 0 0', ACCENT, TEXT, MUTED, BG);
  const d2 = dropCell(content.drops?.dataScience, 'For Data Scientists', '0 20px',     ACCENT, TEXT, MUTED, BG);
  const d3 = dropCell(content.drops?.uiUx,        'For UI/UX Designers', '0 0 0 20px', ACCENT, TEXT, MUTED, BG);
...
<head>
  ...
  <style>
    @media only screen and (max-width:600px) {
      ...
      .drop-col      { display:block !important; width:100% !important; padding:20px 0 !important; border-bottom:1px solid #ddd5c8 !important; }
      .drop-divider  { display:none  !important; }
    }
  </style>
</head>
