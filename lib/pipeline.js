...
const FEEDS = [
  ...
  { url: 'https://www.theverge.com/ai-artificial-intelligence/rss/index.xml', source: 'The Verge', filterAI: true },
];

const AI_KEYWORDS = /\b(ai|model|gpt|claude|gemini|llm)\b/i;
...
function matchesAI(item) {
  return AI_KEYWORDS.test(item.title || '') || AI_KEYWORDS.test(item.contentSnippet || '') || AI_KEYWORDS.test(item.content || '');
}
...
async function fetchFeed({ url, source, filterAI = false, timeout }) {
  try {
    const p = timeout ? new Parser({ timeout }) : parser;
...
export async function fetchArticles() {
  ...
  const allItems = results.flatMap((r) =>
    r.status === 'fulfilled' ? r.value.items.map((item) => ({ item, source: r.value.source })) : []
  );
...
Return this exact JSON structure:
{
  ...
  "drops": {
    "coder": {
      "tool": "string",
      "stack": "string",
      "prompt": "string (starter prompt they paste into Claude/ChatGPT to begin building)"
    },
    "dataScience": {
      "tool": "string",
      "stack": "string",
